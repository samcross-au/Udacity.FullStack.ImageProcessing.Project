...
import { getFullImagePath, getThumbImagePath, fileExists, ImageProperties } from '../routes/api/images';
...
describe('get Full image path', (): void => {
  ...
  it('should return an incorrect Full image path', (): void => {
    ...
    const incorrectPath = path.join(THUMB_PATH, `${imageProperties.filename}.jpg`);
    const result = getFullImagePath(imageProperties);
...
describe('get Thumb image path', (): void => {
  ...
  it('should not return the correct Thumb image path', (): void => {
    const expectedPath = path.join(THUMB_PATH, `${imageProperties.filename}_${imageProperties.width}x${imageProperties.height}_thumb.jpg`);
  ...
  it('should not return an incorrect Thumb image path', (): void => {
    const incorrectPath = path.join(FULL_PATH, `${imageProperties.filename}.jpg`);
    const result = getThumbImagePath(imageProperties);
